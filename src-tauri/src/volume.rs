...
use crate::models::{DiagnosticEvent, PresenceSnapshot};
...
pub(crate) fn sync_chatmix(
    ...
    for change in changes {
        ...
        if config.debug.chatmix_dry_run {
            push_event(
                state,
                DiagnosticEvent::info(format!("dry-run {message}")).category("chatmix"),
            );
        } else {
            push_event(state, DiagnosticEvent::info(message).category("chatmix"));
            state
                .audio
...
pub(crate) fn log_chatmix_wheel(state: &AppState, source: &str, game: Option<u8>, chat: Option<u8>) {
